...
public class PassportApplet extends Applet implements ISO7816 {
    ...
    // 根据你卡的能力选择模式；你之前就是 JCOP41_MODE，这里沿用
    new PassportApplet(PassportCrypto.CREF_MODE).register(); 
    // 如果你想显式用实例 AID 注册，也可以用：
    // short aidLen = (short)(buffer[offset] & 0xFF);
    // new PassportApplet(JCOP41_MODE).register(buffer, (short)(offset + 1), aidLen);
}
    ...
    private void processPutData(APDU apdu) {
        ...
        if (p1 == 0xde && p2 == 0xad) {
			...
			// Reset volatile state to allow fresh BAC authentication
			resetVolatileState();
    ...
    public static void setNoMutuallyAuthenticated() {
        if ((volatileState[0] & MUTUAL_AUTHENTICATED) == MUTUAL_AUTHENTICATED) {
            volatileState[0] ^= MUTUAL_AUTHENTICATED;
        }
    }

    public static void resetVolatileState() {
        volatileState[0] = 0;
    }
